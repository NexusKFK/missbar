...
enum MenuBarItemService {
    ...
    private static let releaseName = "com.nexuskfk.missbar.\(suffix)"
    ...
    /// builds the app as `com.nexuskfk.missbar.debug` while this constant named the release
    /// service outright. Both builds then asked launchd for the one name
    /// `com.nexuskfk.missbar.MenuBarItemService`, so an installed release and a local debug build
    ...
    /// each: the app is `com.nexuskfk.missbar[.debug]`, the service is that plus `.\(suffix)`.
    ...
    /// `Ice.xcodeproj` (`com.nexuskfk.missbar.MenuBarItemService` in Release,
    /// `com.nexuskfk.missbar.debug.MenuBarItemService` in Debug) — that identifier is what launchd
